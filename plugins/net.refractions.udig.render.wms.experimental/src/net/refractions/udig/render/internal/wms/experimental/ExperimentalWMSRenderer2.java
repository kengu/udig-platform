 ...
 *
 */
package net.refractions.udig.render.internal.wms.experimental;
...
import net.refractions.udig.render.wms.experimental.preferences.PreferenceConstants;
 ...
 * </p>
 */
public class ExperimentalWMSRenderer2 extends RendererImpl implements IMultiLayerRenderer {

    private static final String REFRESH_JOB = Messages.ExperimentalWMSRenderer2_refreshJob_title;
     ...
     */
    public ExperimentalWMSRenderer2() {
        ...
        try {
            ...
            if (ExperimentalWMSPlugin.isDebugging(Trace.RENDER)) {
...
//            requestBBox = calculateRequestBBox(wmsLayers, viewport, requestCRS);
            ...
            if (ExperimentalWMSPlugin.isDebugging(Trace.RENDER)) {
                ExperimentalWMSPlugin.trace("Viewport CRS: " + getViewportCRS().getName()); //$NON-NLS-1$
                ExperimentalWMSPlugin.trace("Request CRS: " + requestCRS.getName()); //$NON-NLS-1$
                ExperimentalWMSPlugin.trace("Context bounds: " + getContext().getImageBounds()); //$NON-NLS-1$
                ExperimentalWMSPlugin.trace("Request bounds: " + requestBBox); //$NON-NLS-1$
                ExperimentalWMSPlugin.trace("Backprojected request bounds: " + backprojectedBBox); //$NON-NLS-1$
            }
...
//                    .getDisplaySize(), maxDimensions, getViewportBBox(), backprojectedBBox);
            ...
            if (image == null) {
                Exception e = new RuntimeException(
                        Messages.ExperimentalWMSRenderer2_unable_to_decode_image);
    ...
    private void setFilter( WebMapServer wms, GetMapRequest request ) {
        ...
        for( Map.Entry<ILayer, Filter> entry : filters.entrySet() ) {
            ...
            try {
                ...
                ExperimentalWMSPlugin.log("Error writing filter for layer: " + entry.getKey().getID(), e); //$NON-NLS-1$
    ...
    private void setImageFormat( WebMapServer wms, GetMapRequest request ) {
        ...
        String str = getPreferencesStore().getDefaultString(PreferenceConstants.P_IMAGE_TYPE_ORDER);
        ...
        Thread thread = new Thread(){
            ...
            public void run() {
                ...
                try {
                	...
                	String message = Messages.ExperimentalWMSRenderer2_errorObtainingImage;
                    ...
                    if (inputStream != null)
                        try {
                            ...
                            ExperimentalWMSPlugin.log("failed to close input stream!!!", e); //$NON-NLS-1$
                        }
                }
            }
        };
    ...
    public static Dimension calculateImageDimensions( Dimension displaySize,
        ...
        ExperimentalWMSPlugin.trace("WMS request image dimensions: " + width + ", " + height); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public static ReferencedEnvelope calculateRequestBBox( List<Layer> wmsLayers,
        ...
        if (isEnvelopeNull(layersBBox)) {
            ...
            ExperimentalWMSPlugin.log("Zero width/height envelope: wmsLayers = " + layersBBox); //$NON-NLS-1$
            layersBBox = null;
        ...
        if (isEnvelopeNull(reprojectedViewportBBox)) {
            ...
            ExperimentalWMSPlugin.log("Zero width/height envelope: reprojected viewport from " + viewport //$NON-NLS-1$
        ...
        if (isEnvelopeNull(interestBBox)) {
            ...
            ExperimentalWMSPlugin.trace("Bounds of the data are outside the bounds of the viewscreen."); //$NON-NLS-1$
            return NILL_BOX;
        }
                    ...
                    public void run() {
                        ...
                        ProgressMonitorDialog dialog = new ProgressMonitorDialog(shell);
                            try {
                                ...
                                ExperimentalWMSPlugin.log("Error tracking down EPSG Code", e);
    ...
    private List<ILayer> getLayers() {
        ...
        if (contexts.length == 0)
            throw new RuntimeException(Messages.ExperimentalWMSRenderer2_no_layers_to_render);
        ...
        if (layers.isEmpty()) {
            ExperimentalWMSPlugin.log("WARNING: WMS Renderer contains no layers."); //$NON-NLS-1$
        }
    ...
    Job refreshJob = new Job(REFRESH_JOB){
        ...
        protected IStatus run( IProgressMonitor monitor ) {
            ...
            try {
                ...
                ExperimentalWMSPlugin.log(e.getLocalizedMessage(), e);
            }
    ...
    private IPreferenceStore getPreferencesStore() {
        return ExperimentalWMSPlugin.getDefault().getPreferenceStore();
    }
