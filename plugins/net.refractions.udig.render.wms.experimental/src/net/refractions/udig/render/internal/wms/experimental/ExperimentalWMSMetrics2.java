 ...
 */
public class ExperimentalWMSMetrics2 extends AbstractRenderMetrics {
     ...
     * @param factory
     */
    public ExperimentalWMSMetrics2( IRenderContext context2, ExperimentalWMSMetricsFactory2 factory) {
        super(context2, factory, listExpectedStyleIds());
    }

    public Renderer createRenderer() {
        Renderer renderer=new ExperimentalWMSRenderer2();
	...
	public double getUserAppearanceMetric(IStyleBlackboard blackboard) {
		// Trick uDig into using the experimental renderer?
    	if(ExperimentalWMSPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.P_USE_EXPERIMENTAL_RENDERER))
    	{
    		return 1;
    	}
		return 0;
    ...
    public boolean canAddLayer( ILayer layer ) {
        ...
        try {
            ...
            ExperimentalWMSPlugin.log("", e1); //$NON-NLS-1$
            return false;
        }
        for( IRenderContext renderContext: contexts) {
            ...
            try {
                ...
                ExperimentalWMSPlugin.log("Error while retrieving service.", e); //$NON-NLS-1$
                return false;
            }
            
            if( ExperimentalWMSRenderer2.findRequestCRS(owsLayers, context.getCRS(), context.getMap())==null )
                return false;
